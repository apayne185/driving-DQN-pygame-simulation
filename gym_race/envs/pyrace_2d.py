...
class Car:
    ...
    def draw_radar(self, screen):
        for pos, _ in self.radars: # or self.radars_for_draw
            # pos, dist = r
    ...
    def check_collision(self, map=None):
        ...
        #         self.is_alive = False
        #         break
        self.is_alive = all(self.pixel_at(int(p[0]), int(p[1])) != (255, 255,255, 255) for p in self.four_points)
    ...
    def check_checkpoint(self):
        ...
        self.cur_distance = math.hypot(p[0] - self.center[0], p[1] - self.center[1])
        if self.cur_distance < 70:
            self.current_check = (self.current_check + 1) % len(check_point)
            self.check_flag = True
            self.goal = self.current_check == 0
    ...
    def update(self,map=None):
        self.speed = max(min(self.speed - 0.5, 10), 1)
    ...
    def get_data(self):
        ...
        #     ret[i] = int(r[1] / 30)
        # return ret
        return [r[1] / 200.0 for r in self.radars]
...
class PyRace2D:
    ...
    def action(self, action):
        ...
        if action == 0:   
            self.car.speed = min(self.car.speed + 2, 10)
        elif action == 1:
            self.car.angle += 5
        elif action == 2:
            self.car.angle -= 5
        elif action == 3:            #brake
            self.car.speed = max(self.car.speed - 3, 1)
